...
const ApproveLeave = () => {
 ...
 async function handleDecision(leave:Person,decision:"approved"|"rejected"){
...
if(error){
 ...
 const {error:err} =await SupabaseClient.from("leave_requests").update({status:decision , remark:remark}).eq("id",leave.id);
if(err){
  console.log(err);
}
